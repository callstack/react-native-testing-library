...
- [`toBeChecked()`](/docs/api/jest-matchers#tobechecked) matcher works only on `Switch` host elements or accessibility elements with `checkbox`, `radio` or `switch` role
