...
In v14, `render` is now async by default and returns a Promise. The previous synchronous `render` function has been renamed to `deprecated_renderSync`. This change makes it compatible with React 19, React Suspense, and `React.use()`.
...
To ease migration, we provide `deprecated_renderSync` which maintains the same synchronous behavior as v13. This allows you to migrate gradually.

#### `deprecated_renderSync` {#deprecated-render-sync}
...
```ts
// Old v13 code (still works but deprecated)
import { deprecated_renderSync } from '@testing-library/react-native';

it('should render component', () => {
  deprecated_renderSync(<MyComponent />);
  expect(screen.getByText('Hello')).toBeOnTheScreen();
});
```
...
To migrate from `deprecated_renderSync` to `render`:
