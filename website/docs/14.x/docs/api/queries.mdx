...
When `aria-labelledby` or `accessibilityLabelledBy` references multiple elements, their text content is joined with spaces in the referenced order and matched as a single label.
