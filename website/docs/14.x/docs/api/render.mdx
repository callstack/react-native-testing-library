...
When using `render`, you have to use corresponding lifecycle methods: `rerenderAsync` and `unmountAsync` instead of their sync versions.

:::

## `deprecated_renderSync` function {#deprecated-render-sync}
...
```tsx
function deprecated_renderSync(
  component: React.Element<any>,
  options?: DeprecatedRenderOptions
): DeprecatedRenderResult;
```

The `deprecated_renderSync` function is the synchronous version of [`render`](#render). It was the default `render` function in previous versions but is now deprecated in favor of the async `render` function.

```jsx
import { deprecated_renderSync, screen } from '@testing-library/react-native';

test('sync component test', () => {
  deprecated_renderSync(<MyApp />);
  expect(screen.getAllByRole('button', { name: 'start' })).toBeOnTheScreen();
});
```
...
`deprecated_renderSync` accepts the same options as `render`.

### Result

The `deprecated_renderSync` function returns the same queries and utilities as the [`screen`](docs/api/screen) object. We recommended using the `screen` object as more developer-friendly way.

:::note Migration

To migrate from `deprecated_renderSync` to `render`, see the [migration guide](/docs/migration/v14#render-is-now-async-by-default).
