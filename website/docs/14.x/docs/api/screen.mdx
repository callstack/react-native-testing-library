...
Returns a pseudo-element container whose children are the elements you asked to render. This is the root container element from React Test Renderer.
...
test('example', async () => {
  await render(<View testID="root-view"><Text>Hello</Text></View>);
