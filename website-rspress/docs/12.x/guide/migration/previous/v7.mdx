...
1. There are only 3 short-hand events: [`fireEvent.press`](api/events/fire-event#press), [`fireEvent.changeText`](api/events/fire-event#change-text) and [`fireEvent.scroll`](api/events/fire-event#scroll). For all other or custom events you can use the base signature.
