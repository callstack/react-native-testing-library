...
If you are already using legacy `@testing-library/jest-native` Jest Matchers, we have a [migration guide](guide/migration/jest-matchers) for moving to the built-in matchers.
...
Previous behaviour of matching hidden elements can be enabled on query level using [includeHiddenElements](api/core/queries#includehiddenelements-option) query options or globally using `defaultIncludeHiddenElements`(api#defaultincludehiddenelements-option) configuration option.
...
`*ByText`, `*ByDisplayValue`, `*ByPlaceholderText` queries now return [host elements](guide/advanced/testing-env#host-and-composite-components), which is consistent with other queries.
