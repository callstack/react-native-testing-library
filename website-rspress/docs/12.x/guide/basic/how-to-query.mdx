...
| ---------------------------------------------- | ----------------------------- | ------------------------------------------ | --------- |
| [`getBy*`](api/core/queries#get-by)            | Exactly one matching element  | `ReactTestInstance`                        | No        |
| [`getAllBy*`](api/core/queries#get-all-by)     | At least one matching element | `Array<ReactTestInstance>`                 | No        |
| [`queryBy*`](api/core/queries#query-by)        | Zero or one matching element  | <code>ReactTestInstance &#124; null</code> | No        |
| [`queryAllBy*`](api/core/queries#query-all-by) | No assertion                  | `Array<ReactTestInstance>`                 | No        |
| [`findBy*`](api/core/queries#find-by)          | Exactly one matching element  | `Promise<ReactTestInstance>`               | Yes       |
| [`findAllBy*`](api/core/queries#find-all-by)   | At least one matching element | `Promise<Array<ReactTestInstance>>`        | Yes       |
...
4. Use `queryBy*` only when element **should not exist** to use it together with e.g. [`not.toBeOnTheScreen()`](api/jest-matchers#tobeonthescreen) matcher.
...
| ------------------------------------------------------------ | ------------------ | ------------------------------------------------------------------------------------------- |
| [`*ByRole`](api/core/queries#by-role)                        | all host elements  | `role`, `accessibilityRole`,<br /> optional: accessible name, accessibility state and value |
| [`*ByLabelText`](api/core/queries#by-label-text)             | all host elements  | `aria-label`, `aria-labelledby`,<br /> `accessibilityLabel`, `accessibilityLabelledBy`      |
| [`*ByDisplayValue`](api/core/queries#by-display-value)       | `TextInput`        | `value`, `defaultValue`                                                                     |
| [`*ByPlaceholderText`](api/core/queries#by-placeholder-text) | `TextInput`        | `placeholder`                                                                               |
| [`*ByText`](api/core/queries#by-text)                        | `Text`             | `children` (text content)                                                                   |
| [`*ByHintText`](api/core/queries#by-hint-text)               | all host elements  | `accessibilityHint`                                                                         |
| [`*ByTestId`](api/core/queries#by-test-id)                   | all host elements  | `testID`                                                                                    |
...
Frequently, you will want to add the [`name`](api/core/queries#by-role-options) option, which will match both the element's role and its accessible name (= element's accessibility label or text content).
...
1. [`*ByLabelText`](api/core/queries#by-label-text) - will match the accessibility label of the element. This query will match any host elements, including `TextInput` elements.
2. [`*ByPlaceholderText`](api/core/queries#by-placeholder-text) - will match the placeholder of `TextInput` element. This query will match only `TextInput` elements.
3. [`*ByDisplayValue`](api/core/queries#by-display-value) - will the current (or default) value of `TextInput` element. This query will match only `TextInput` elements.
...
- [`*ByText`](api/core/queries#by-text) - will match the text content of the element. This query will match only `Text` elements.
- [`*ByLabelText`](api/core/queries#by-label-text) - will match the accessibility label of the element.
- [`*ByHintText`](api/core/queries#by-hint-text) - will match the accessibility hint of the element.

### 4. Test ID query {#test-id-query}

As a final predicate, you can use the `testID` prop to find relevant views. Using the [`*ByTestId`](api/core/queries#by-test-id) predicate offers the most flexibility, but at the same time, it does not represent the user experience, as users are not aware of test IDs.
