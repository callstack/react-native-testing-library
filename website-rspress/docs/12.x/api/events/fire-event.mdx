...
:::note
For common events like `press` or `type` it's recommended to use [User Event API](api/events/user-event) as it offers
...
:::note
It is recommended to use the User Event [`press()`](api/events/user-event#press) helper instead as it offers more realistic simulation of press interaction, including pressable support.
:::
...
:::note
It is recommended to use the User Event [`type()`](api/events/user-event#type) helper instead as it offers more realistic simulation of text change interaction, including key-by-key typing, element focus, and other editing events.
:::
...
Prefer using [`user.scrollTo`](api/events/user-event#scrollto) over `fireEvent.scroll` for `ScrollView`, `FlatList`, and `SectionList` components. User Event provides a more realistic event simulation based on React Native runtime behavior.
