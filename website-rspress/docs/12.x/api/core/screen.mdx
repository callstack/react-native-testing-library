...
This object is assigned after the `render` call and cleared after each test by calling [`cleanup`](api/misc/other#cleanup). If no `render` call has been made in a given test, then it holds a special object and throws a helpful error on each property and method access.
...
See [Queries](api/core/queries) for a complete list.
...
Returns the rendered root [host element](guide/advanced/testing-env#host-and-composite-components).
...
Returns the rendered [composite root element](guide/advanced/testing-env#host-and-composite-components).
