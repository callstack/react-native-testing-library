...
The `render` function returns the same queries and utilities as the [`screen`](api/core/screen) object. We recommended using the `screen` object as more developer-friendly way.
