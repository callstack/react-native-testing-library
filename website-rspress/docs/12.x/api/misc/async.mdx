...
The `findBy*` queries are used to find elements that are not instantly available but will be added as a result of some asynchronous action. Learn more details [here](api/core/queries#find-by).
...
:::note
If you receive warnings related to `act()` function consult our [Undestanding Act](guide/advanced/understanding-act.md) function document.
:::
