...
`within` (also available as `getQueriesForElement` alias) performs [queries](api/core/queries) scoped to given element.
...
Consult our [Undestanding Act function](guide/advanced/understanding-act.md) document for more understanding of its intricacies.
